...
class Location extends Record {
    const TABLE_NAME = 'fnblocation';

     /**
     * Find a Location by id
     *
     * @param int $id
     * @return mixed A Location or false on failure.
     */
    public static function findById($id) {
        return self::findByIdFrom('Location', $id);
    }

    public static function findBy($column, $value) {
        return Record::findOneFrom('Location', $column.' = ?', array($value));
    }
    ...
    public static function find($args = null) {
		...
		$tablename = self::tableNameFromClassName('Location');
		...
		if ($limit == 1) {
			...
			$objects = array();
			while ($object = $stmt->fetchObject('Location')) {
				$objects[] = $object;
			}
			return $objects;
		}
    ...
    /**
     * Find a Location by its feature id.
     *
     * @param string $id
     * @return mixed A Location or false on failure.
     */
    public static function findByFnbId($id) {
        $where = self::tableNameFromClassName('Location').'.fnbid='.(int)$id;
