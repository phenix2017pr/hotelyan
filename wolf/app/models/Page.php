...
class Page extends Node {
	...
	public function part_content($part) {
        ob_start();
		eval('?>'.$part->content_html);
		$out = ob_get_contents();
		ob_end_clean();
		return $out;
    }
    ...
    public function extlink($label=null, $options='',$extlink='')
	{
		if ($label == null)
			$label = $this->title();
    ...
    public function children($args=null, $value=array(), $include_hidden=false) {
        ...
        // Collect attributes...
        $where   = isset($args['where']) ? $args['where']: '';
        $order   = isset($args['order']) ? $args['order']: 'location, page.position, page.id';
        $offset  = isset($args['offset']) ? $args['offset']: 0;
        $limit   = isset($args['limit']) ? $args['limit']: 0;
    ...
    public static function childrenOf($id) {
        return self::find(array('where' => 'parent_id='.$id, 'order' => 'position, page.created_on ASC'));
    }
