 ...
 *
 */
class FeatureImage extends Record {
    const TABLE_NAME = 'featureimage';

     /**
     * Find a FeatureImage by id
     *
     * @param int $id
     * @return mixed A FeatureImage or false on failure.
     */
    public static function findById($id) {
        return self::findByIdFrom('FeatureImage', $id);
    }

    public static function findBy($column, $value) {
        return Record::findOneFrom('FeatureImage', $column.' = ?', array($value));
    }
    ...
    public static function find($args = null) {
		...
		$tablename = self::tableNameFromClassName('FeatureImage');
		...
		// Run!
		if ($limit == 1) {
			return $stmt->fetchObject('FeatureImage');
		}
		else {
			$objects = array();
			while ($object = $stmt->fetchObject('FeatureImage')) {
				$objects[] = $object;
			}
			return $objects;
		}
    ...
    /**
     * Find a FeatureImage by its feature id.
     *
     * @param string $id
     * @return mixed A FeatureImage or false on failure.
     */
    public static function findByRoomId($id) {
        $where = self::tableNameFromClassName('FeatureImage').'.roomid='.(int)$id;
