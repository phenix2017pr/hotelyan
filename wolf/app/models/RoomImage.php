 ...
 *
 */
class RoomImage extends Record {
    const TABLE_NAME = 'roomimage';
    ...
    */
     ...
     /**
     * Find a RoomImage by id
     *
     * @param int $id
     * @return mixed A RoomImage or false on failure.
     */
    public static function findById($id) {
        return self::findByIdFrom('RoomImage', $id);
    }

    public static function findBy($column, $value) {
        return Record::findOneFrom('RoomImage', $column.' = ?', array($value));
    }
    ...
    public static function find($args = null) {
		...
		$tablename = self::tableNameFromClassName('RoomImage');
		...
		// Run!
		if ($limit == 1) {
			return $stmt->fetchObject('RoomImage');
		}
		else {
			$objects = array();
			while ($object = $stmt->fetchObject('RoomImage')) {
				$objects[] = $object;
			}
			return $objects;
		}
    ...
    /**
     * Find a RoomImage by its room id.
     *
     * @param string $id
     * @return mixed A RoomImage or false on failure.
     */
    public static function findByRoomId($id) {
        $where = self::tableNameFromClassName('RoomImage').'.roomid='.(int)$id;
    ...
    public function getColumns() {
        return array('id', 'roomid', 'reference_code', 'url', 'tagcontent', 'publish', 'created_on', 'updated_on', 'created_by_id', 'updated_by_id');
    }
    */
}