 ...
 *
 */
class OfferImage extends Record {
    const TABLE_NAME = 'offerimage';
    ...
    */
     ...
     /**
     * Find a OfferImage by id
     *
     * @param int $id
     * @return mixed A OfferImage or false on failure.
     */
    public static function findById($id) {
        return self::findByIdFrom('OfferImage', $id);
    }

    public static function findBy($column, $value) {
        return Record::findOneFrom('OfferImage', $column.' = ?', array($value));
    }
    ...
    public static function find($args = null) {
		...
		$tablename = self::tableNameFromClassName('OfferImage');
		...
		// Run!
		if ($limit == 1) {
			return $stmt->fetchObject('OfferImage');
		}
		else {
			$objects = array();
			while ($object = $stmt->fetchObject('OfferImage')) {
				$objects[] = $object;
			}
			return $objects;
		}
    ...
    /**
     * Find a OfferImage by its offer id.
     *
     * @param string $id
     * @return mixed A OfferImage or false on failure.
     */
    public static function findByOfferId($id) {
        $where = self::tableNameFromClassName('OfferImage').'.offerid='.(int)$id;
    ...
    public function getColumns() {
        return array('id', 'offerid', 'reference_code', 'url', 'tagcontent', 'publish', 'created_on', 'updated_on', 'created_by_id', 'updated_by_id');
    }
    */
}