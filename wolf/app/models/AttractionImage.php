 ...
 *
 */
class AttractionImage extends Record {
    const TABLE_NAME = 'attractionimage';
    ...
    */
     ...
     /**
     * Find a AttractionImage by id
     *
     * @param int $id
     * @return mixed A AttractionImage or false on failure.
     */
    public static function findById($id) {
        return self::findByIdFrom('AttractionImage', $id);
    }

    public static function findBy($column, $value) {
        return Record::findOneFrom('AttractionImage', $column.' = ?', array($value));
    }
    ...
    public static function find($args = null) {
		...
		$tablename = self::tableNameFromClassName('AttractionImage');
		...
		// Run!
		if ($limit == 1) {
			return $stmt->fetchObject('AttractionImage');
		}
		else {
			$objects = array();
			while ($object = $stmt->fetchObject('AttractionImage')) {
				$objects[] = $object;
			}
			return $objects;
		}
    ...
    /**
     * Find a AttractionImage by its attraction id.
     *
     * @param string $id
     * @return mixed A AttractionImage or false on failure.
     */
    public static function findByAttractionId($id) {
        $where = self::tableNameFromClassName('AttractionImage').'.attractionid='.(int)$id;
    ...
    public function getColumns() {
        return array('id', 'attractionid', 'reference_code', 'url', 'tagcontent', 'publish', 'created_on', 'updated_on', 'created_by_id', 'updated_by_id');
    }
    */

    public static function getLastAttractionImageSeq($vid) {
        $where = self::tableNameFromClassName('AttractionImage').'.attractionid='.(int)$vid;
        $last = self::find(array(
			'where' => $where,
			'order' => 'sequence DESC',
			'limit' => 1
        ));
