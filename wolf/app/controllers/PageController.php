...
class PageController extends Controller {
    ...
    public function edit($id) {
		...
		// get homepage info - customization for HattenHotel
		 $pagesetting = array();
		 if ($id == 1){
		 	$pagesetting = PageSetting::init();
		 }
    ...
    private function _store($action, $id=false) {
        ...
        if ($page->save()) {
			...
			// save homepage info - hatten hotel customization
			// if ($id == 1) {
			// 	// upload hotel image 1, 2 & 3
			// 	if(isset($_FILES) && !empty($_FILES['upload_highlight_image']['name'])){
			// 		$file = $this->upload_file($_FILES['upload_highlight_image']['name'], FILES_DIR.'/pagesetting/images/', $_FILES['upload_highlight_image']['tmp_name'], $overwrite);
			// 		if ($file === false) {
			// 			Flash::set('error', __('Highlight image could not be uploaded!'));
			// 			redirect(get_url('page/edit/1'));
			// 		} else {
			// 			$pagesetting['highlight_image'] = $file;
			// 		}
			// 	}
